...
def run_massive_system_pipeline(input_sdf, work_dir, box_tensor=None):
    ...
    write_gro_file_stream(output_gro_path, coords, global_res_names, global_res_ids, box_tensor)
...
def write_gro_file_stream(path, coords, res_names, res_ids, box_tensor):
    ...
    with open(path, 'w', buffering=65536) as f:
        ...
        for i in range(num_atoms):
            ...
            f.write(
                f"{res_id_safe:5d}{str(res_names[i])[:5]:<5}{f'A{i + 1}'[:5]:>5}{atom_id_safe:5d}{coords[i][0] / 10.0:8.3f}{coords[i][1] / 10.0:8.3f}{coords[i][2] / 10.0:8.3f}\n")
